...
/**
 * MyTable — now a "dumb" renderer for server-paginated data.
 *
 * Props:
 * - columns, data: same as before. `data` is expected to be ONLY the
 *   current page's rows (whatever the API returned), not the full dataset.
 * - currentPage, pageCount, pageSize: pagination state owned by the parent.
 * - onPageChange(newPage): called when user clicks prev/next/first/last.
 * - onPageSizeChange(newSize): called when user changes the page size select.
 * - totalCount: optional, shown in the footer.
 */
const MyTable = ({
    columns,
    data,
    ...
    const {
        ...
        headerGroups,
        prepareRow,
        rows, // all rows of `data` — data is already just one server page
    } = useTable(
        {
            columns,
            data,
    ...
    const goToFirst = () => canPreviousPage && onPageChange?.(1);
    const goToPrevious = () => canPreviousPage && onPageChange?.(currentPage - 1);
    const goToNext = () => canNextPage && onPageChange?.(currentPage + 1);
    const goToLast = () => canNextPage && onPageChange?.(pageCount);
    ...
    return (
        <div className="w-full bg-white rounded-xl border border-blue-100 shadow-sm overflow-hidden mt-6 text-neutral">
            ...
            {/* Pagination — driven by server state via props */}
            <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-3 border-t border-gray-300">
                <div className="flex items-center justify-between w-full">
                    <div className="flex items-center sm:gap-2 gap-0.5">
                        <button
