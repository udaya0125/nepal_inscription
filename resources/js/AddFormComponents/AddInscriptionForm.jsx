...
const AddInscriptionForm = ({ showForm, setShowForm, setReloadTrigger }) => {
    ...
    const convertToNepaliNumerals = (num) => {
        ...
        return num.toString().split('').map(digit => nepaliNumerals[parseInt(digit)] || digit).join('');
    };

    // Function to convert ordered lists to Nepali numerals
    const convertListsToNepali = (editor) => {
        if (!editor) return;
        const lists = editor.root.querySelectorAll('ol');
        lists.forEach(list => {
            list.setAttribute('data-nepali', 'true');
            const items = list.querySelectorAll('li');
            items.forEach((item, idx) => {
                const nepaliNumber = convertToNepaliNumerals(idx + 1);
                item.setAttribute('data-nepali-num', nepaliNumber);
            });
        });
    };
    ...
    // Custom Quill modules for Devnagari text with Nepali numerals
    const devTextQuillModules = {
        toolbar: [
            [{ header: [1, 2, 3, 4, 5, 6, false] }],
            ["bold", "italic", "underline", "strike"],
            [{ list: "ordered" }, { list: "bullet" }],
            ...
            'list': function(value) {
                ...
                if (range) {
                    if (value === 'ordered') {
                        ...
                        setTimeout(() => {
                            convertListsToNepali(this.quill);
                        }, 10);
                    } else {
    ...
    const quillPlaceholders = {
        ...
        dev_text: "Enter the Devnagari text in numbered format (१. पहिलो लाइन, २. दोस्रो लाइन, ३. तेस्रो लाइन)...",
    ...
    // Effect to convert existing lists to Nepali numerals when dev_text loads
    useEffect(() => {
        if (quillRefs.dev_text.current) {
            const editor = quillRefs.dev_text.current.getEditor();
            setTimeout(() => {
                convertListsToNepali(editor);
            }, 100);
        }
    }, []);
                                    ...
                                    render={({ field }) => (
                                        <div className="rounded overflow-hidden">
                                            <ReactQuill
                                                ...
                                                onBlur={field.onBlur}
                                                modules={devTextQuillModules}
                                                formats={quillFormats}
                                                style={{
                                                    ...quillStyle("dev_text"),
                                                    fontFamily: "'Noto Sans Devanagari', 'Noto Sans Siddham', sans-serif"
                                                }}
                                                className="rounded h-auto quill-custom dev-text-editor"
