      ...
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          {trend !== undefined && trend !== 0 && (
            <p className={`text-xs mt-1.5 font-medium ${trend > 0 ? 'text-green-600' : 'text-red-600'}`}>
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50">
          {data.map((p, i) => {
            const share = total ? ((p.value / total) * 100).toFixed(2) : "0";
...
const Dashboard = () => {
  ...
  const getTrend = (data, key) => {
    ...
    const previous7 = data.slice(-14, -7).reduce((s, d) => s + d[key], 0) / 7;
    return previous7 ? Math.round(((recent7 - previous7) / previous7) * 100) : 0;
  };
  ...
  // Peak day
  const peakDay = timeData.length > 0 ? timeData.reduce(
